    ...
    <main class="flex-1 overflow-auto">
        ...
        <header
            ...
            <div class="block">
                ...
                <div class="flex items-center justify-between px-5">
                    <span class="text-xl text-gray-600 hidden lg:block">អតិថិជនដែលបានបញ្ចប់ចងការប្រាក់</span>
                </div>
            </div>
        ...
        <div class="p-6 khmer__font">
            <div class="flex items-center justify-between">
                <nav aria-label="Breadcrumb" class="">
                    <ol class="flex items-center gap-1 text-sm text-gray-600">
                        ...
                        <li>
                            <a href="#" class="block transition-colors hover:text-teal-500"> អតិថិជនដែលបានបញ្ចប់ចងការប្រាក់ </a>
                        </li>
                    </ol>
                </nav>
            ...
            <div class="header__btn__search flex items-center justify-between mt-6">
                <span class="text-xl text-gray-600">អតិថិជនដែលបានបញ្ចប់ចងការប្រាក់ទាំងអស់</span>
                <div class="search__header flex items-center">
                    <label for="Search">
                        <div class="box__search relative w-[300px] mr-2">
                            ...
                            <input type="text" placeholder="ឈ្មោះអតិថិជន" id="endLoanInput"
                                class="mt-0.5 w-full rounded-lg border-gray-200 px-10 h-10 text-sm drop-shadow mt-1 border-none sm:text-sm focus:outline-none focus:ring-teal-500 focus:ring-2" />
                        </div>
                    </label>
                    <button
                        class="flex items-center rounded-md bg-teal-600 mt-2 px-4 h-10 text-sm drop-shadow font-medium text-white transition hover:scale-110 hover:shadow-xl focus:ring-3 focus:outline-hidden"
                    ...
                    <button
                        class="flex items-center rounded-md bg-red-500 mt-2 ml-2 px-4 h-10 text-sm drop-shadow font-medium text-white transition hover:scale-110 hover:shadow-xl focus:ring-3 focus:outline-hidden"
            ...
            <div class="table__loan mt-10">
                    ...
                    <table class="w-full bg-white  overflow-hidden shadow-md">
                        <thead class="sticky khmer__font top-0 text-gray-600 text-sm">
                            <tr >
                                ...
                                <th class="p-4 text-left">ស្ថានភាព</th>
                                <th class="p-4 text-left">សកម្មភាព</th>
                            </tr>
                        </thead>
                        <tbody class="text-gray-700" id="endLoansTable">
                ...
                <div class="pagination__container flex items-center justify-between mt-5">
                    ...
                    <div class="pagination__num flex justify-center gap-1 text-gray-900">
                        ...
                        <div id="pagination-loan-num" style="margin-top: -1px;"></div>
...
@push('scripts_multiple')
            <script>
                ...
                function showModal(endLoanId) {
                    const deleteModal = document.getElementById('deleteModal');
                    const deleteForm = document.getElementById('deleteForm');

                    // Set the form action with the loan ID
                    deleteForm.action = `/application/admin/loan/delete/${endLoanId}`;
                ...
                // Function fetch data from backend to display in frontend endLoans
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    let rowsPerPage = 10;
                    let endLoans = [];
                    let filteredendLoans = [];

                    try {
                        endLoans = @json($endLoans);
                        console.log('endLoans data:', endLoans);
                        filteredendLoans = [...endLoans];
                        console.log('Filtered endLoans:', filteredendLoans);
                        displayPage(1);
                    } catch (error) {
                    ...
                    const endLoanInput = document.getElementById('endLoanInput');
                    const searchButton = document.getElementById('search');
                    const clearButton = document.getElementById('clear');

                    endLoanInput.addEventListener('input', () => {
                        if (endLoanInput.value.length >= 2) {
                            filterTable();
                        } else {
                            resetTable();
                        }
                    ...
                    function displayPage(page) {
                        ...
                        currentPage = page;
                        const totalItems = filteredendLoans.length;
                        const totalPages = Math.ceil(totalItems / rowsPerPage);
                        console.log('Total items:', totalItems, 'Total pages:', totalPages);

                        const start = (page - 1) * rowsPerPage;
                        const end = Math.min(start + rowsPerPage, totalItems);
                        const paginatedData = filteredendLoans.slice(start, end);
                        console.log('Paginated data:', paginatedData);

                        const tableBody = document.getElementById('endLoansTable');
                        if (!tableBody) {
                        ...
                        if (totalItems === 0) {
                            tableBody.innerHTML = '<tr><td colspan="14" class="p-4 text-center">មិនមានអតិថិជនបានបញ្ចប់ចងការប្រាក់ទេ</td></tr>';
                        } else {
                            paginatedData.forEach((endLoan, index) => {
                                console.log(`Rendering endLoan ${index}:`, endLoan);
                                ...
                                switch (endLoan.loan.currency) {
                                    case "riel":
                                        loanAmount = endLoan.loan.loan_Amount_Riel ? endLoan.loan.loan_Amount_Riel : "មិនមាន";
                                        interestRate = endLoan.loan.interest_Rate_Riel ? endLoan.loan.interest_Rate_Riel : "មិនមាន";
                                        paymentAmount = endLoan.loan.payment_Amount_Riel ? endLoan.loan.payment_Amount_Riel : "មិនមាន";
                                        currencySymbol = "រៀល";
                                        break;
                                    case "baht":
                                        loanAmount = endLoan.loan.loan_Amount_Baht ? endLoan.loan.loan_Amount_Baht : "មិនមាន";
                                        interestRate = endLoan.loan.interest_Rate_Baht ? endLoan.loan.interest_Rate_Baht : "មិនមាន";
                                        paymentAmount = endLoan.loan.payment_Amount_Baht ? endLoan.loan.payment_Amount_Baht : "មិនមាន";
                                        currencySymbol = "បាត";
                                        break;
                                    case "dollar":
                                        loanAmount = endLoan.loan.loan_Amount_Dollar ? endLoan.loan.loan_Amount_Dollar : "មិនមាន";
                                        interestRate = endLoan.loan.interest_Rate_Dollar ? endLoan.loan.interest_Rate_Dollar : "មិនមាន";
                                        paymentAmount = endLoan.loan.payment_Amount_Dollar ? endLoan.loan.payment_Amount_Dollar : "មិនមាន";
        ...
        <td class="p-4 text-left text-sm">${endLoan.loan.client.full_Name}</td>
        ...
        <td class="p-4 text-left text-sm">${endLoan.loan.loan_Term} ថ្ងៃ</td>
        <td class="p-4 text-left text-sm">${paymentAmount} ${currencySymbol}</td>
        <td class="p-4 text-left text-sm">${convertToKhmerNumbers(new Date(endLoan.loan.start_Date).toLocaleDateString())}</td>
        <td class="p-4 text-left text-sm">${convertToKhmerNumbers(new Date(endLoan.loan.end_Date).toLocaleDateString())}</td>
        <td class="p-4 text-left text-sm">${endLoan.status?.toLowerCase() === 'paid' ?
                                        `<span class="inline-flex items-center justify-center rounded-sm bg-emerald-100 px-2.5 py-0.5 text-emerald-700 dark:bg-emerald-700 dark:text-emerald-100">
                ...
                <p class="text-sm whitespace-nowrap">បង់រួច</p>
                                        ...
                                        `<span class="inline-flex items-center justify-center rounded-sm bg-amber-100 px-2.5 py-0.5 text-amber-700 dark:bg-amber-700 dark:text-amber-100">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="-ms-1 me-1.5 size-4">
        ...
        <td class="p-4 text-left text-sm">
            <span class="inline-flex divide-x divide-gray-300 overflow-hidden rounded border border-gray-300 bg-white shadow-sm">
                <a href="/application/admin/end/view-end-loan/${endLoan.id}" class="px-3 py-1.5 text-sm font-medium text-sky-600 transition-colors hover:bg-gray-50 hover:text-sky-600 focus:relative" aria-label="View">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                    </svg>
                </a>
                <a href="#" onclick="showModal('${endLoan.id}')" class="px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-gray-50 hover:text-red-600 focus:relative" aria-label="Delete">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
    ...
    `;
                    ...
                    function updatePaginationControls(totalPages) {
                        const paginationContainer = document.getElementById('pagination-loan-num');
                    ...
                    function goToPage(page) {
                        const totalPages = Math.ceil(filteredendLoans.length / rowsPerPage);
                    ...
                    function filterTable() {
                        const searchText = document.getElementById('endLoanInput').value.toLowerCase();

                        // If search is empty or less than 2 characters, show all data
                        if (searchText.length < 2) {
                            filteredendLoans = [...endLoans];
                        } else {
                            filteredendLoans = endLoans.filter(loan =>
                                loan.loan.client.full_Name.toLowerCase().includes(searchText)
                            );
                        }
