    ...
    <style>
        ...
        @include('application.message')
                <div class="p-6 khmer__font">
                    <div class="flex items-center justify-between">
                        <nav aria-label="Breadcrumb" class="">
                            <ol class="flex items-center gap-1 text-sm text-gray-600">
                                ...
                                <li>
                                    <a href="#" class="block transition-colors hover:text-teal-500"> អតិថិជនដែលចងការប្រាក់ </a>
                                </li>
                            </ol>
                        </nav>
                        <a class="flex items-center rounded-md bg-teal-600 px-4 h-10 drop-shadow-md text-sm font-medium text-white transition hover:scale-110 hover:shadow-xl focus:ring-3 focus:outline-hidden"
                            href="{{ route('application.pages.loans.add-loan') }}">
                            ...
                            ចុះឈ្មោះចងការប្រាក់ថ្មី
                        </a>
                    </div>
                    <div class="header__btn__search flex items-center justify-between mt-6">
                        <span class="text-xl text-gray-600">អតិថិជនដែលចងការប្រាក់ទាំងអស់</span>
                        <div class="search__header flex items-center">
                            <label for="Search">
                                <div class="box__search relative w-[300px] mr-2">
                                    ...
                                    <input type="text" placeholder="ឈ្មោះអតិថិជន" id="loanInput" class="mt-0.5 w-full rounded-lg border-gray-200 px-10 h-10 text-sm drop-shadow mt-1 border-none sm:text-sm focus:outline-none focus:ring-teal-500 focus:ring-2" />
                                </div>
                            </label>
                            <button class="flex items-center rounded-md bg-teal-600 mt-2 px-4 h-10 text-sm drop-shadow font-medium text-white transition hover:bg-teal-600 hover:scale-110 hover:shadow-xl focus:ring-3 focus:outline-hidden"
                            ...
                            <button class="flex items-center rounded-md bg-red-500 mt-2 ml-2 px-4 h-10 text-sm drop-shadow font-medium text-white transition hover:bg-red-500 hover:scale-110 hover:shadow-xl focus:ring-3 focus:outline-hidden"
                    ...
                    <div class="flex items-center justify-center flex-wrap w-full gap-4 mt-6">
                        <!-- Base - Right -->
                        <a class="inline-block rounded-sm bg-teal-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:rotate-2 focus:ring-3 focus:outline-hidden"
                            ...
                            កម្ចីសាច់ប្រាក់ទាំងអស់
                        </a>

                        <!-- Base - Left -->
                        <a class="inline-block rounded-sm bg-teal-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:-rotate-2 focus:ring-3 focus:outline-hidden"
                            href="{{ route('application.pages.loans.loan-baht') }}">
                            កម្ចីសាច់ប្រាក់ជាលុយបាត
                        </a>

                        <!-- Border - Right -->
                        <a class="inline-block rounded-sm bg-teal-600 px-8 py-3 text-sm font-medium text-white transition hover:scale-110 hover:rotate-2 focus:ring-3 focus:outline-hidden"
                            href="{{ route('application.pages.loans.loan-dollar') }}">
                            កម្ចីសាច់ប្រាក់ជាលុយដុល្លា
                        </a>
                    </div>
                    <div class="table__loan mt-6">
...
@push('scripts_multiple')
    <script>
        ...
        function showModal(loanId) {
            ...
            deleteForm.action = `/application/admin/loan/delete/${loanId}`;
        ...
        // Function fetch data from backend to display in frontend loans
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function displayPage(page) {
                ...
                const paginatedData = filteredLoans.slice(start, end);
                ...
                if (totalItems === 0) {
                    tableBody.innerHTML = '<tr><td colspan="14" class="p-4 text-center">មិនមានអតិថិជនចងការប្រាក់ទេ</td></tr>';
                } else {
                   paginatedData.forEach((loan, index) => {
                        console.log(`Rendering loan ${index}:`, loan);
                        ...
                        switch (loan.currency) {
                            case "riel":
                                loanAmount = loan.loan_Amount_Riel ? loan.loan_Amount_Riel : "មិនមាន";
                                interestRate = loan.interest_Rate_Riel ? loan.interest_Rate_Riel : "មិនមាន";
                                paymentAmount = loan.payment_Amount_Riel ? loan.payment_Amount_Riel : "មិនមាន";
                                currencySymbol = "រៀល";
                                break;
                            case "baht":
                                loanAmount = loan.loan_Amount_Baht ? loan.loan_Amount_Baht : "មិនមាន";
                                interestRate = loan.interest_Rate_Baht ? loan.interest_Rate_Baht : "មិនមាន";
                                paymentAmount = loan.payment_Amount_Baht ? loan.payment_Amount_Baht : "មិនមាន";
                                currencySymbol = "បាត";
                                break;
                            case "dollar":
                                loanAmount = loan.loan_Amount_Dollar ? loan.loan_Amount_Dollar : "មិនមាន";
                                interestRate = loan.interest_Rate_Dollar ? loan.interest_Rate_Dollar : "មិនមាន";
                                paymentAmount = loan.payment_Amount_Dollar ? loan.payment_Amount_Dollar : "មិនមាន";
        ...
        <td class="p-4 text-left text-sm">${loan.client.full_Name}</td>
        ...
        <td class="p-4 text-left text-sm">${loan.loan_Term} ថ្ងៃ</td>
        <td class="p-4 text-left text-sm">${paymentAmount} ${currencySymbol}</td>
        <td class="p-4 text-left text-sm">${convertToKhmerNumbers(new Date(loan.start_Date).toLocaleDateString())}</td>
        <td class="p-4 text-left text-sm">${convertToKhmerNumbers(new Date(loan.end_Date).toLocaleDateString())}</td>
        <td class="p-4">
            <img alt="" src="/src/assets/uploads/loans/pictures/${loan.picture}" class="w-auto h-auto rounded object-cover" onerror="this.src='/src/assets/default-image.jpg'" />
        </td>
        <td class="p-4">
            <video alt="" src="/src/assets/uploads/loans/videos/${loan.video}" class="w-auto h-auto rounded object-cover" onerror="this.outerHTML='<span>មិនមានវីដេអូ</span>'" />
        </td>
        <td class="p-4 text-left text-sm">${convertToKhmerNumbers(new Date(loan.created_at).toLocaleDateString())}</td>
        <td class="p-4 text-left text-sm">
            <span class="inline-flex divide-x divide-gray-300 overflow-hidden rounded border border-gray-300 bg-white shadow-sm">
                <a href="/application/admin/loan/view/${loan.id}" class="px-3 py-1.5 text-sm font-medium text-sky-600 transition-colors hover:bg-gray-50 hover:text-sky-600 focus:relative" aria-label="View">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                    </svg>
                </a>
                <a href="/application/admin/loan/edit/${loan.id}" class="px-3 py-1.5 text-sm font-medium text-teal-600 transition-colors hover:bg-gray-50 hover:text-teal-900 focus:relative" aria-label="Edit">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125"/>
                    </svg>
                </a>
                <a href="#" onclick="showModal('${loan.id}')" class="px-3 py-1.5 text-sm font-medium text-red-600 transition-colors hover:bg-gray-50 hover:text-red-600 focus:relative" aria-label="Delete">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="size-4">
    ...
    `;
            ...
            function filterTable() {
                const searchText = document.getElementById('loanInput').value.toLowerCase();
                if (searchText.length < 2) return;

                filteredLoans = loans.filter(loan =>
                    loan.client.full_Name.toLowerCase().includes(searchText)
                );
                displayPage(1);
            }
